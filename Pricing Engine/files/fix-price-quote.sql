-- FIX: replaces price_quote() with array_append() for the flags list.
-- Safe to run on the existing database — it replaces the function in place.
...
begin
  ...
  if p_mode = 'ltl' and cube > 0 then
    chargeable := greatest(actual, cube * 1728.0 / r.dim_divisor);
    any_dim := (cube * 1728.0 / r.dim_divisor) > actual;
  end if;
