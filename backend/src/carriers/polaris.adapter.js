...
// permanent limitation of their API, not a build choice — a domestic LTL quote always falls
// back to the mock, same as envelope/parcel always did before this integration.
class PolarisAdapter extends CarrierAdapter {
  ...
  async getRates(params) {
    ...
    if (params.shipmentType !== 'ltl') return this._getMockRates(params);
    const scope = detectScope(params.origin.country, params.destination.country);
    if (scope !== 'xb') return this._getMockRates(params);

    if (this.isLive) {
      try {
        return await this._getLiveRates(params);
      } catch (err) {
        console.error('[POLARIS-RATE] Live rates failed, falling back to mock:', err.message);
  ...
  async getTracking(trackingNumber) {
    if (this.isLive) {
      try {
        ...
        console.error('[POLARIS-TRACK] Live tracking failed, falling back to mock:', err.message);
  ...
  async bookShipment(details) {
    if (this.isLive) {
      try {
        ...
        console.error('[POLARIS-SHIP] Live booking failed, falling back to mock:', err.message);
  ...
  _mockBookShipment(errorMessage) {
    return {
      carrierTrackingNumber: `POL-${new Date().getFullYear()}-${String(Date.now()).slice(-5)}`,
      confirmationNumber: `POL-CONF-${Date.now()}`,
      status: 'confirmed',
      label: null,
